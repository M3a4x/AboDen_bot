...
client.on('ready', function(){ // Leaked by [ @M3a4x ]
    var ms = 30000 ;
var setGame = [`Al AboDen Bot `,`$help | $invites`];
    var i = -1;
    var j = 0;
...
client.on('message', message => { // Leaked by [ @M3a4x ]
    ...
    translate(finalToTrans, {to: language}).then(res => {
            message.channel.send({embed: {
                color: 3447003,
                author: {
                  name: 'Al AboDen Bot\'s translator',
...
.setTitle('Al AboDen Bot Info')
.setURL('https://discordapp.com/api/oauth2/authorize?client_id=500205140922597376&permissions=0&scope=bot')
...
var message = "zg";
         const embed = new Discord.RichEmbed()
     .setColor("GOLD")
     .setTitle('Al AboDen Bot Link Click Here .!')
     .setURL('https://discord.gg/hp8RNSY')
  .setDescription(`**
  New Server Add Al AboDen Bot ✅
...
var message = "zg";
     ...
     .setURL('https://discord.gg/hp8RNSY')
  .setDescription(`**
  Server Kicked Al AboDen Bot :cry:
اسم السيرفر: ${guild.name}
صاحب السيرفر: ${guild.owner}**`);
client.channels.get("465340887279468564").sendEmbed(embed)
});
...
if (message.content.startsWith('-bcall')){
 if (message.author.id !== '202745501345382400') return message.reply('** هذا الأمر قفط لصاحب البوت و شكراًً **')
 if(!message.author.id === '202745501345382400') return;
